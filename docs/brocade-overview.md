...
[```Brocade Master ZIP```](store/brocade-08-11-2021.zip)  
```ICX6xxx SW version: 08030u```  
```ICX7xxx SW version: 08095d```  
```ZIP Updated: 08-11-2021```  
```MD5: 161d3ab1965fbb7d66c01af9c6e0916d```  

Inside this archive you'll see a folder titled `brocade-(date)` - extract this entire folder somewhere on your PC. Then, you need to get a TFTP server up and running temporarily, serving files out of the `TFTP Content` subfolder inside the folder you just extracted. If you're already familiar with how to do this, point your TFTP server at `TFTP Content`, so it's the root folder, then navigate to your switch model under the **Brocade Setup** menu on the left. If not, continue below:

### Setting up TFTP (Windows)
For a simple temporary TFTP server under Windows, I highly recommend Tftpd64. In the brocade ZIP above, a portable copy that requires no installation is included under the `Tftpd64` folder - just launch the `tftpd64.exe` executable inside - it should already be configured with the appropriate settings, and set to serve the correct folder (`..\TFTP Content\`). You may have to click the "Browse" button on the right of the program to select the `TFTP Content` folder if you've changed settings or moved the `Tftpd64` folder out of the main folder.  

Otherwise, that's it! You're now serving all the files via TFTP so your switch can grab them. When done with the guide, just exit the program. If you don't want to trust an executable from a random guide site, you can also just [download Tftpd64](https://pjo2.github.io/tftpd64/) from the official site and configure it yourself (granted, if you `diff` it with mine, you'll see they're identical).
...
```
That's it, you're now serving all the guide files and your switches can grab them. If you have trouble, you should see a good amount of related info from `tail -F /var/log/syslog` - see if it's complaining about file permissions or similar. When finished with the guide, you can stop the daemon and it won't start again until you tell it to, or just uninstall it completely:
```
