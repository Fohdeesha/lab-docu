...
**Connect** to the switches serial/console port on the front using a program like Putty, and connect any of the normal switch ports to your network (do NOT use the dedicated management port).
...
#### Mini-USB-B Serial Cable

The details of the mini-USB-B serial cable and port from [Ruckus' website](http://docs.ruckuswireless.com/fastiron/hardware/icx7250-installguide/GUID-A333FDA4-68CA-46D1-BB78-4A2B7F1DA37C.html) are:
...
If you would like to create this cable, make sure the mini-USB cable is a "data" cable, not just a charging cable.  

One could use a simple FTDI USB-to-Serial adapter since it shows up on Windows, macOS, and Linux as a simple serial port with no drivers (Windows built-in drivers).  Also, try to get one with bare pins showing GNX, TX, and RX inputs as it's easy to twist, solder, breadboard, etc.

Next, find an old mini-USB "data" cable, cut it open, connect the black GND to GND.  TX/RX will usually be Green (TX) and White (RX) in color - connect them to the FTDI pins for RX and TX respectfully (as the FDTI pins are labelled for itself).  **If nothing shows up**, just try reversing the RX/TX cables and boot the device again.
